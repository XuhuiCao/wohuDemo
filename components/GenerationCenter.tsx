...
export const GenerationCenter: React.FC<GenerationCenterProps> = ({ onBack }) => {
  const stats = [
    { label: '提示词数量', count: 48, bg: 'bg-[#FFF2F2]' },
    { label: '评估器数量', count: 48, bg: 'bg-[#FFF2F2]' },
    { label: '数据集数量', count: 48, bg: 'bg-[#FFF2F2]' },
  ];
    ...
    <div className="flex-1 h-full bg-white flex flex-col font-sans">
      ...
      <div className="h-14 bg-white border-b border-gray-100 flex items-center px-6 justify-between flex-shrink-0">
        <div className="flex items-center gap-2 text-sm text-gray-500">
             ...
             <span className="font-medium text-gray-900">生成中心</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-8 bg-[#FDF4F4]/20 custom-scrollbar">
        <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="grid grid-cols-3 gap-6">
                {stats.map((stat, idx) => (
                    <div key={idx} className={`${stat.bg} p-6 rounded-xl border border-[#FBEAEA] shadow-sm flex flex-col gap-4`}>
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-gray-800">生成提示词</span>
            ...
            <div className="min-h-[500px] bg-[#FDF4F4]/30 rounded-2xl border border-dashed border-[#FBEAEA] flex flex-col items-center justify-center text-gray-400">
                ...
                <p className="text-sm">暂无生成的提示词项目，快去新建一个吧</p>
            </div>
        </div>
      </div>
    </div>
